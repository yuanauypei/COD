...
class NeighborConnectionDecoder(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        # pdb.set_trace()
        x1_1 = x1
        unsample_x1 = F.interpolate(x1, scale_factor=2, mode='bilinear', align_corners=True)
        x2_1 = self.conv_upsample1(unsample_x1) * x2

        upsample_x2_1 = F.interpolate(x2_1, scale_factor=2, mode='bilinear', align_corners=True)
        upsample_x2 = F.interpolate(x2, scale_factor=2, mode='bilinear', align_corners=True)

        x3_1 = self.conv_upsample2(upsample_x2_1) * self.conv_upsample3(upsample_x2) * x3
        upsample_x1_1 = F.interpolate(x1_1, scale_factor=2, mode='bilinear', align_corners=True)

        x2_2 = torch.cat((x2_1, self.conv_upsample4(upsample_x1_1)), 1)
        x2_2 = self.conv_concat2(x2_2)
        
        upsample_x2_2 = F.interpolate(x2_2, scale_factor=2, mode='bilinear', align_corners=True)

        x3_2 = torch.cat((x3_1, self.conv_upsample5(upsample_x2_2)), 1)
...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        return S_g_pred.sigmoid(), S_5_pred.sigmoid(), S_4_pred.sigmoid(), S_3_pred.sigmoid()
