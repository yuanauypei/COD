...
class Network(nn.Module):
    ...
    def __init__(self, channel=32, imagenet_pretrained=True):
        ...
        self.upsample_S_g_pred = nn.Upsample(scale_factor=8, mode='bilinear', align_corners=True)
        self.upsample_guidance_g = nn.Upsample(scale_factor=0.25, mode='bilinear', align_corners=True)
        self.upsample_S_5_pred = nn.Upsample(scale_factor=32, mode='bilinear', align_corners=True)
        self.upsample_guidance_5 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.upsample_S_4_pred = nn.Upsample(scale_factor=16, mode='bilinear', align_corners=True)
        self.upsample_guidance_4 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.upsample_S_3_pred = nn.Upsample(scale_factor=8, mode='bilinear', align_corners=True)
    ...
    def forward(self, x):
        ...
        S_g_pred = self.upsample_S_g_pred(S_g)    # Sup-1 (bs, 1, 44, 44) -> (bs, 1, 352, 352)

        # ---- reverse stage 5 ----
        guidance_g = self.upsample_guidance_g(S_g)
        ra4_feat = self.RS5(x4_rfb, guidance_g)
        S_5 = ra4_feat + guidance_g
        S_5_pred = self.upsample_S_5_pred(S_5)  # Sup-2 (bs, 1, 11, 11) -> (bs, 1, 352, 352)

        # ---- reverse stage 4 ----
        guidance_5 = self.upsample_guidance_5(S_5)
        ra3_feat = self.RS4(x3_rfb, guidance_5)
        S_4 = ra3_feat + guidance_5
        S_4_pred = self.upsample_S_4_pred(S_4)  # Sup-3 (bs, 1, 22, 22) -> (bs, 1, 352, 352)

        # ---- reverse stage 3 ----
        guidance_4 = self.upsample_guidance_4(S_4)
        ra2_feat = self.RS3(x2_rfb, guidance_4)
        S_3 = ra2_feat + guidance_4
        S_3_pred = self.upsample_S_3_pred(S_3)   # Sup-4 (bs, 1, 44, 44) -> (bs, 1, 352, 352)
