...
def structure_loss(pred, mask):
    ...
    """
    # 考虑使用平方加剧对不平横区域的关注
    # kernel size的设计是否合理 是否可以通过多层的 kernel size 进一步改进 weit的形式
...
def structure_loss(pred, mask):
    ...
    weit = 1 + 5 / 3 * torch.abs(F.avg_pool2d(mask, kernel_size=31, stride=1, padding=15) +
                                 F.avg_pool2d(mask, kernel_size=15, stride=1, padding=7) +
                                 F.avg_pool2d(mask, kernel_size=7, stride=1, padding=3) -
                                 3 * mask)
    weit = weit ** 2
    wbce = F.binary_cross_entropy_with_logits(pred, mask, reduce='none')
    wbce = (weit * wbce).sum(dim=(2, 3)) / weit.sum(dim=(2, 3))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str,
                        default='./snapshot/SINet_V2_loss*3_ori/',
