...
def adjust_lr(optimizer, epoch, decay_rate=0.1, decay_epoch=30):
    decay = (1-decay_rate) ** (epoch // decay_epoch)
    for param_group in optimizer.param_groups:
        param_group['lr'] *= decay

def trainer(train_loader, model, optimizer, epoch, opt, loss_func, total_step, device):
    """
    Training iteration
    :param train_loader:
    :param model:
    ...
    for step, data_pack in enumerate(train_loader):
        ...
        pdb.set_trace()
        pdb.set_trace()
        aaa = images.cpu().clone()
        aaa = aaa.squeeze(0)#delete batch_size
