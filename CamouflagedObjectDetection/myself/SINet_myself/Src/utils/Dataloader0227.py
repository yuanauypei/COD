...
class CamObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, trainsize):
        ...
        self.HVF_transform = transforms.Compose([
            transforms.RandomHorizontalFlip(p=1),
            transforms.RandomVerticalFlip(p=1)
        ])

        self.VF_transform = transforms.Compose([
    ...
    def __getitem__(self, index):
        ...
        indicator = random.randint(1, 4)
        if indicator == 1:
            image = self.HF_transform(image)
            gt = self.HF_transform(gt)
        elif indicator == 2:
            image = self.VF_transform(image)
            gt = self.VF_transform(gt)
        elif indicator == 3:
            image = self.HVF_transform(image)
            gt = self.HVF_transform(gt)
        else:
            pass

        indicator = random.randint(1, 5)
        add_matrix = torch.ones_like(image[0,:,:])
        if indicator == 1:
            image = image[0,:,:]
            image = torch.stack((image, add_matrix, add_matrix), dim=0)
        elif indicator == 2:
            image = image[1,:,:]
            image = torch.stack((add_matrix, image, add_matrix), dim=0)
        elif indicator == 3:
            image = image[2,:,:]
            image = torch.stack((add_matrix, add_matrix, image), dim=0)
        elif indicator == 4:
