...
class CELoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        numerator = torch.pow((pred - intersection), 2).sum() + torch.pow((target - intersection), 2).sum()
        denominator = torch.pow(pred, 2).sum() + torch.pow(target, 2).sum()
...
def parse():
    ...
    parser.add_argument('--save_model', type=str, default='./Snapshot/2020-CVPR-SINet-0227/')
